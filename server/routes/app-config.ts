...
import { authMiddleware, getAppScope, requireAppPermission } from "../lib/auth.js";
...
export function registerAppConfigRoutes(app: express.Application): void {
	app.get(
		...
		authMiddleware,
		requireAppPermission("update", getAppScope),
		async (req, res) => {
	...
	app.post("/api/apps/:name/config", authMiddleware, async (req, res) => {
		const name = getParam(req.params, "name");
	...
	app.delete("/api/apps/:name/config/:key", authMiddleware, async (req, res) => {
		const name = getParam(req.params, "name");
