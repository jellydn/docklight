...
describe("AuditPagination", () => {
	describe("rendering", () => {
		it("should return null when total is 0", () => {
			const { container } = render(<AuditPagination total={0} offset={0} setOffset={vi.fn()} />);
		...
		it("should render pagination when total exceeds ITEMS_PER_PAGE", () => {
			render(<AuditPagination total={ITEMS_PER_PAGE + 1} offset={0} setOffset={vi.fn()} />);
		...
		it("should show Page 1 of 2 when total is 51 and offset is 0", () => {
			render(<AuditPagination total={51} offset={0} setOffset={vi.fn()} />);
			expect(screen.getByText(`Page 1 of 2`)).toBeInTheDocument();
		});

		it("should show Page 2 of 2 when on the second page", () => {
			render(<AuditPagination total={51} offset={ITEMS_PER_PAGE} setOffset={vi.fn()} />);
			expect(screen.getByText(`Page 2 of 2`)).toBeInTheDocument();
		});

		it("should show correct page info with multiple pages", () => {
			render(<AuditPagination total={150} offset={ITEMS_PER_PAGE} setOffset={vi.fn()} />);
			expect(screen.getByText(`Page 2 of 3`)).toBeInTheDocument();
		});

		it("should display result range for first page", () => {
			render(<AuditPagination total={100} offset={0} setOffset={vi.fn()} />);
		...
		it("should display result range for second page", () => {
			render(<AuditPagination total={75} offset={ITEMS_PER_PAGE} setOffset={vi.fn()} />);
	...
	describe("Previous button", () => {
		...
		it("should be enabled when offset is greater than 0", () => {
			render(<AuditPagination total={100} offset={ITEMS_PER_PAGE} setOffset={vi.fn()} />);
			const prevButtons = screen.getAllByRole("button", { name: "Previous" });
			for (const btn of prevButtons) {
		...
		it("should call setOffset with decreased value when clicked", async () => {
			...
			render(<AuditPagination total={150} offset={ITEMS_PER_PAGE} setOffset={setOffset} />);
			const prevButtons = screen.getAllByRole("button", { name: "Previous" });
		...
		it("should never call setOffset with negative value", async () => {
			...
			render(<AuditPagination total={100} offset={10} setOffset={setOffset} />);
			const prevButtons = screen.getAllByRole("button", { name: "Previous" });
			await user.click(prevButtons[0]);
	...
	describe("Next button", () => {
		...
		it("should be disabled when offset + ITEMS_PER_PAGE equals total", () => {
			render(<AuditPagination total={100} offset={50} setOffset={vi.fn()} />);
			const nextButtons = screen.getAllByRole("button", { name: "Next" });
			for (const btn of nextButtons) {
				expect(btn).toBeDisabled();
			}
		...
		it("should call setOffset with increased value when clicked", async () => {
			const user = userEvent.setup();
			const setOffset = vi.fn();
			render(<AuditPagination total={150} offset={0} setOffset={setOffset} />);
			const nextButtons = screen.getAllByRole("button", { name: "Next" });
		...
		it("should advance to the correct page offset when on page 2", async () => {
			const user = userEvent.setup();
			const setOffset = vi.fn();
			render(<AuditPagination total={200} offset={ITEMS_PER_PAGE} setOffset={setOffset} />);
			const nextButtons = screen.getAllByRole("button", { name: "Next" });
			await user.click(nextButtons[0]);
	...
	describe("theme token CSS classes", () => {
		it("should use bg-muted/50 for pagination container (not bg-gray-50)", () => {
			const { container } = render(<AuditPagination total={100} offset={0} setOffset={vi.fn()} />);
